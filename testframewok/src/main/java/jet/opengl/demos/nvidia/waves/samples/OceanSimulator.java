...
public class OceanSimulator implements WaterWaveSimulator{
	...
	private Texture2D m_ptex2DDisplacementMap;
	private Texture2D m_ptex2DGradientMap;
...
//					memset(zero_data, 0, 3 * output_size * sizeof(float) * 2);
	...
	private void initHeightMap(OceanParameter params, Vector2f[] out_h0, float[] out_omega){
		int i, j;
...
//		GL11.glTexParameteri(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_WRAP_T, GL11.GL_REPEAT);
	...
	public Texture2D getDisplacementMap() {
		if(m_ptex2DDisplacementMap == null){
			m_ptex2DDisplacementMap = TextureUtils.createTexture2D(GLenum.GL_TEXTURE_2D, m_pDisplacementMap);
		}
		return m_ptex2DDisplacementMap;
	}

	@Override
	public Texture2D getGradMap() {
		if(m_ptex2DGradientMap == null) {
			m_ptex2DGradientMap = TextureUtils.createTexture2D(GLenum.GL_TEXTURE_2D, m_pGradientMap);
		}

		return m_ptex2DGradientMap;
	}
