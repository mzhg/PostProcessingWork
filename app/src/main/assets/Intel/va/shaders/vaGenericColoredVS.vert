...
void main()
{
    ...
    ret.ViewspacePos            = mul( g_Global.View, float4( Position.xyz, 1) );
    ret.ViewspaceNormal.xyz     = normalize( mul( g_Global.View, float4(Normal.xyz, 0.0) ).xyz );
    ret.ViewspaceTangent.xyz    = normalize( mul( g_Global.View, float4(Tangent.xyz, 0.0) ).xyz );
