...
float4 DIFFUSE( /*PS_INPUT input*/ )
{
    return
#ifdef _CPUT
//texture_DM.Sample( SAMPLER0, input.UV0 )
texture(texture_DM, _input.UV0)
#else
...
float4 SPECULAR( /*PS_INPUT input*/ )
{
    return
#ifdef _CPUT
//texture_SM.Sample( SAMPLER0, input.UV0 )
texture(texture_SM, _input.UV0)
#else
tex2D( SAMPLER2, input.UV0 )
#endif
;
}
...
float4 NORMAL( /*PS_INPUT input*/ )
{
    return ((
#ifdef _CPUT
//texture_NM.Sample( SAMPLER0, input.UV0 )
texture(texture_NM, _input.UV0)
#else
...
void main()
{
	...
	// float  OpaqueshadowAmount = ComputeShadowAmount(input);
