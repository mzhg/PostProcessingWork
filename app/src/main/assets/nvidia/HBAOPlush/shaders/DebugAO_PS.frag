...
//    float HardwareDepth = LinearDepthTexture.SampleLevel(PointClampSampler, UV, 0);
	float HardwareDepth = textureLod(LinearDepthTexture, UV, 0.0).r;
...
//    float4 Rand = RandomTexture.Sample(PointWrapSampler, IN.pos.xy / RANDOM_TEXTURE_WIDTH);
	float4 Rand = texture(RandomTexture, IN.pos.xy / RANDOM_TEXTURE_WIDTH);
#else
...
#if ENABLE_BLUR
    OutColor.xy = float2(AO, BlurDepth);
