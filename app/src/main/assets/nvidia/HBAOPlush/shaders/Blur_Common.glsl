...
float2 PointSampleAODepth(float2 UV)
{
    return texture(g_t0, UV).xy;
}
float2 LinearSampleAODepth(float2 UV)
{
    return texture(g_t1, UV).xy;
}

#else

layout(binding=0) uniform sampler2D AODepthTexturePoint;
layout(binding=1) uniform sampler2D AODepthTextureLinear;

float2 PointSampleAODepth(float2 UV)
{
    return texture(AODepthTexturePoint, UV).xy;
}
float2 LinearSampleAODepth(float2 UV)
{
    return texture(AODepthTextureLinear, UV).xy;
}
#endif
...
#else
//    [unroll]
    for (float R = R0; R <= KERNEL_RADIUS; R += 1.0)
    {
        float2 UV = R * DeltaUV + Center.UV;
        float2 AOZ = PointSampleAODepth(UV);
        ProcessSample(AOZ, R, DepthSlope, Center, TotalAO, TotalW);
...
//-------------------------------------------------------------------------
float ComputeBlur(float2 IN_UV,
                  float2 DeltaUV,
                  out float CenterDepth)
{
    float2 AOZ = PointSampleAODepth(IN_UV);
    CenterDepth = AOZ.y;
