...
void main()
{
    ...
    float sceneZ = textureLod(sceneDepthTex, normalizedInputPos,0).r;  // samLinearClamp
    ...
    float rayDataBackDepth = textureLod(rayDataTex, float2(gl_FragCoord.x/RTWidth, gl_FragCoord.y/RTHeight),0).w;  // samLinearClamp
    ...
    OutColor.w = rayDataBackDepth - inputDepth;
}