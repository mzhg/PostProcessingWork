...
uniform float4 ShadowBufferSize;
...
uniform float4x4 ShadowViewProjectionMatrices[6];

uniform float InvShadowmapResolution;

float SampleCmpLevelZero(float3 uv, float ref_depth)
{
    return texture(ShadowDepthCubeTexture2, float4(uv, ref_depth), 0.0);
}
...
float CubePCF3x3Quarter(float3 NormalizedLightVector, float3 SideVector, float3 UpVector, float CompareDistance, float Bias = 0)
{
    ...
    ShadowResults0.x = SampleCmpLevelZero( Sample00Coordinate, CompareDistance + Bias * length(float2(0.5f, 0.5f)));
    ShadowResults0.y = SampleCmpLevelZero( Sample01Coordinate, CompareDistance + Bias * length(float2(0.5f, 1.5f)));
    ShadowResults0.z = SampleCmpLevelZero( Sample02Coordinate, CompareDistance + Bias * length(float2(0.5f, 2.5f)));

    float3 ShadowResults1;
    ShadowResults1.x = SampleCmpLevelZero( Sample10Coordinate, CompareDistance + Bias * length(float2(1.5f, 0.5f)));
    ShadowResults1.y = SampleCmpLevelZero( Sample11Coordinate, CompareDistance + Bias * length(float2(1.5f, 1.5f)));
    ShadowResults1.z = SampleCmpLevelZero( Sample12Coordinate, CompareDistance + Bias * length(float2(1.5f, 2.5f)));

    float3 ShadowResults2;
    ShadowResults2.x = SampleCmpLevelZero( Sample20Coordinate, CompareDistance + Bias * length(float2(2.5f, 0.5f)));
    ShadowResults2.y = SampleCmpLevelZero( Sample21Coordinate, CompareDistance + Bias * length(float2(2.5f, 1.5f)));
    ShadowResults2.z = SampleCmpLevelZero( Sample22Coordinate, CompareDistance + Bias * length(float2(2.5f, 2.5f)));

    return dot(ShadowResults0 + ShadowResults1 + ShadowResults2, float3(.1111111f));
}

const float2 DiscSamples5[]=
...
#if SHADOW_QUALITY <= 2
        Shadow = SampleCmpLevelZero( NormalizedLightVector, CompareDistance + ShadowDepthBias);
#elif SHADOW_QUALITY == 3
        for(int i = 0; i < 5; ++i)
        {
            float3 SamplePos = NormalizedLightVector + SideVector * DiscSamples5[i].x + UpVector * DiscSamples5[i].y;
            Shadow += SampleCmpLevelZero(
