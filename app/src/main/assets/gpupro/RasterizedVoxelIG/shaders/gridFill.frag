...
const float3 faceVectors[4] = float[4]( float3(0.0f,-0.57735026f,0.81649661f),
...
{
	// get surface color
	float4 base = colorMap.Sample(colorMapSampler,_input.texCoords);
...
#ifdef FINE_GRID
    float3 offset = (_input.positionWS-customUB.snappedGridPositions[0].xyz)*customUB.gridCellSizes.y;
#else
	float3 offset = (_input.positionWS-customUB.snappedGridPositions[1].xyz)*customUB.gridCellSizes.w;
#endif
	offset = round(offset);
