...
layout(binding = 0) uniform sampler2DShadow colorMap;
...
float ComputeShadowTerm(in float4 positionWS)
{
  ...
  for(int i=0;i<PCF_NUM_SAMPLES;i++)
  {
	    ...
	    shadowTerm += texture(colorMap,float3(texCoords,result.z-SHADOW_BIAS));// colorMapSampler
  }
...
void main()
{
	...
	int3 voxelPos = int3(gl_FragCoord.xy,gl_Layer);
