...
layout(binding = 0) uniform sampler2D earthcolor;
layout(binding = 1) uniform sampler2D oceanmask;

const float zoom = 2.00;							// camera focal length

// image postprocess
const float exposure = 1.00;						// global image brightness
const float saturation = 1.25;					// color saturation control

// basic parameters
const float R = 1.;								    // planet radius
const float top = 1.2;							// atmosphere top (up to 70 km)
const float H = 0.00119 * 3.;							// density scale-height of atmosphere (not pressure scale height)
const vec3 L = vec3( -1, -0, 0 ); 					// light direction (sun)
...
const vec3 beta = vec3( 38.05, 82.36, 214.65 );

// a small absobtion in the orange band is added
// to simulate the effect of ozone
const vec3 absorb = vec3( 0.75, 0.85, 1. );

// devrived values
const float LN2 = 0.693147181;
const float H50 = H * LN2;
const vec3 beta50 = beta * LN2;
const float invH50 = 1. / H50;
const float X50 = R / H50;
...
uniform vec3 g_CameraView;
// --------------------------------------------------------

void main()
{
	vec3 color = vec3(0);
	vec3 vertex = m_PositionWS;
	vec3 campos = g_CameraPos;
	vec3 zaxis = g_CameraView;
	...
	else
	{
		...
		vec3 surfacecolor = texture( earthcolor, uv ).xyz;		surfacecolor = pow( surfacecolor, vec3( 2.2 ) );
		...
		surfacecolor = texture( oceanmask, uv ).x > 0.45 ? oceancolor : landcolor;
	...
	Out_Color = vec4( pow( color, vec3( .4545 ) ), 1 );
}
