    ...
    switch (g_samplePattern)
    {
        case POISSON_25_25:
        {
            ...
            for (int i = 0; i < 25; ++i)
            {
                ...
                float shadowMapDepth = textureLod(g_shadowMapDepth, uv + offset, 0).r;
        ...
        {
            maxBlockers = 49;
            vec2 stepUV = searchRegionRadiusUV / 3;
            for (int x = -3; x <= 3; ++x)
            {
                for (int y = -3; y <= 3; ++y)
                {
...
float pcfFilter(vec2 uv, float z0, vec2 dz_duv, vec2 filterRadiusUV, sampler2DShadow g_shadowMapPcf)
{
    float sum = 0.0;
    ...
    switch (g_samplePattern)
    {
        case POISSON_25_25:
        {
            for (int i = 0; i < 25; ++i)
            {
                ...
                float z = biasedZ(z0, dz_duv, offset);
                sum += texture(g_shadowMapPcf, vec3(uv + offset, z));
            }
            return sum / 25.0;
        }
...
float pcssShadow(vec2 uv, float z, vec2 dz_duv, float zEye, sampler2DShadow g_shadowMapPcf, sampler2D g_shadowMapDepth)
{
    // ------------------------
    ...
    findBlocker(accumBlockerDepth, numBlockers, maxBlockers, uv, z, dz_duv, searchRegionRadiusUV, g_shadowMapDepth);
    ...
    // STEP 3: filtering
    // ------------------------
    return pcfFilter(uv, z, dz_duv, filterRadius, g_shadowMapPcf);
}

float pcfShadow(vec2 uv, float z, vec2 dz_duv, float zEye, sampler2DShadow g_shadowMapPcf, sampler2D g_shadowMapDepth)
{
    // Do a blocker search to enable early out
    float accumBlockerDepth, numBlockers, maxBlockers;
    vec2 searchRegionRadius = searchRegionRadiusUV(zEye);
    findBlocker(accumBlockerDepth, numBlockers, maxBlockers, uv, z, dz_duv, searchRegionRadius, g_shadowMapDepth);
    if (numBlockers == 0)
        return 1.0;
...
float CaculateShadows(vec3 worldPosition, sampler2DShadow g_shadowMapPcf, sampler2D g_shadowMapDepth)
{
...
//    float z = lightPosition.z / lightPosition.w;
    ...
    vec2 dz_duv = depthGradient(lightProjTexPos.xy, lightProjTexPos.z);
    ...
    {
        case 0:
            shadow = texture(g_shadowMapPcf, lightProjTexPos.xyz);
            break;
        case 1:
            shadow = pcssShadow(lightProjTexPos.xy, lightProjTexPos.z, dz_duv, zEye, g_shadowMapPcf, g_shadowMapDepth);
            break;

        case 2:
            shadow = pcfShadow(lightProjTexPos.xy, lightProjTexPos.z, dz_duv, zEye, g_shadowMapPcf, g_shadowMapDepth);
            break;
    }
