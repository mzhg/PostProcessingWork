...
uint PackParticleIJRing(uint i, uint j, uint ring, uint layer)
{
    return i | (j<<12U) | (ring<<24U) | (layer<<28U);
}

#ifdef __cplusplus
static void UnPackParticleIJRing(uint ID, uint &i, uint &j, uint &ring, uint &layer)
#else
void UnPackParticleIJRing(uint ID, out uint i, out uint j, out uint ring, out uint layer)
#endif
{
    i=(ID)&((1<<12)-1); 
    j=((ID)>>12)&((1<<12)-1); 
