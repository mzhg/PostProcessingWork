...
void main()
{
	...
	uvec3 GTid = gl_LocalInvocationID;   // TODO
    ...
    float2 f2SampleLocationPS = 
//    				g_tex2DCoordinates.Load( uint3(uiGlobalSampleInd, uiSliceInd, 0) );
    				texelFetch(g_tex2DCoordinates, int2(uiGlobalSampleInd, uiSliceInd), 0).rg;
...
//    [branch]
    if( bIsValidThread )
    {
        ...
        float fCamSpaceZ =            
//        					g_tex2DEpipolarCamSpaceZ.Load( uint3(uiGlobalSampleInd,   uiSliceInd, 0) );
        					texelFetch(g_tex2DEpipolarCamSpaceZ, int2(uiGlobalSampleInd,   uiSliceInd), 0).r;
        float fRightNeighbCamSpaceZ = 
//        					g_tex2DEpipolarCamSpaceZ.Load( uint3(uiGlobalSampleInd+1, uiSliceInd, 0) );
        					texelFetch(g_tex2DEpipolarCamSpaceZ, int2(uiGlobalSampleInd+1, uiSliceInd), 0).r;
        float fMaxZ = max(fCamSpaceZ, fRightNeighbCamSpaceZ);
        fMaxZ = max(fMaxZ, 1.0);
        // Compare the difference with the threshold
        bool bFlag = abs(fCamSpaceZ - fRightNeighbCamSpaceZ)/fMaxZ < 0.2*g_fRefinementThreshold;
#elif REFINEMENT_CRITERION == REFINEMENT_CRITERION_INSCTR_DIFF
        // Load inscattering for this sample and for its right neighbour
        float3 f3Insctr0 = 
//        					g_tex2DScatteredColor.Load( uint3(uiGlobalSampleInd,   uiSliceInd, 0) );
							texelFetch(g_tex2DScatteredColor, int2(uiGlobalSampleInd,   uiSliceInd), 0).rgb;
        float3 f3Insctr1 = 
//        					g_tex2DScatteredColor.Load( uint3(uiGlobalSampleInd+1, uiSliceInd, 0) );
        					texelFetch(g_tex2DScatteredColor, int2(uiGlobalSampleInd+1, uiSliceInd), 0).rgb;
        ...
        float3 f3MinInsctrThreshold = (0.02f * fAverageLum.xxx / RGB_TO_LUMINANCE.xyz) / g_fMiddleGray;
        ...
        bool bFlag = all( lessThan((abs(f3Insctr0 - f3Insctr1)/f3MaxInsctr), float3(g_fRefinementThreshold) ));
#endif
        // Set appropriate flag using INTERLOCKED Or:
        atomicOr( g_uiPackedCamSpaceDiffFlags[uiSampleInd/32], bFlag ? 1 << (uiSampleInd%32):0 );
    }
    ...
    float2 f2InitialSample0Coords = 
//    						g_tex2DCoordinates.Load( uint3(uiInitialSample0GlobalInd, uiSliceInd, 0) );
    						texelFetch(g_tex2DCoordinates, int2(uiInitialSample0GlobalInd, uiSliceInd), 0).rg;
    if( float(uiInitialSample0GlobalInd)/float(MAX_SAMPLES_IN_SLICE) < 0.05 && 
        length(f2InitialSample0Coords - g_f4LightScreenPos.xy) < 0.1 )
    {
        uiInitialSampleStep = max( INITIAL_SAMPLE_STEP / g_uiEpipoleSamplingDensityFactor, 1U );
...
//    g_rwtex2DInterpolationSource.GetDimensions(uiInterpolationTexWidth, uiInterpolationTexHeight);
	int2 idemension = imageSize(g_rwtex2DInterpolationSource);
	uiInterpolationTexWidth = uint(idemension.x);
    ...
    if( uiSampleInd > uiInitialSample0Ind && uiSampleInd < uiInitialSample1Ind )
    {
...
#if INITIAL_SAMPLE_STEP < 32
        {
            ...
            int iFlagPackOrder = int(uiInitialSample0Ind / 32);
            int iFlagOrderInPack = int(uiInitialSample0Ind % 32);
        ...
        else
        {
            // Find left interpolation source
            {
                ...
                int iFlagPackOrder = (iFirstDepthBreakToTheLeftInd) / 32;
                int iFlagOrderInPack = (iFirstDepthBreakToTheLeftInd) % 32;
                uint uiFlagPack = uiPackedCamSpaceDiffFlags[iFlagPackOrder];
...
//    g_rwtex2DInterpolationSource[ uint2(uiGlobalSampleInd, uiSliceInd) ] = uint2(uiGroupStartGlobalInd + uiLeftSrcSampleInd, uiGroupStartGlobalInd + uiRightSrcSampleInd);
	imageStore(g_rwtex2DInterpolationSource, int2(uiGlobalSampleInd, uiSliceInd), uint4(
		uiGroupStartGlobalInd + uiLeftSrcSampleInd, uiGroupStartGlobalInd + uiRightSrcSampleInd, 0, 0
	));
}
