...
void DirectionToZenithAzimuthAngleXZY(in float3 f3Direction, out float fZenithAngle, out float fAzimuthAngle)
{
    ...
    fAzimuthAngle = atan(fAzimuthSin, fAzimuthCos);
}
...
{                                                               \
    ...
    f3UVW.z = (fQ0Slice + f4LUTCoords.z) / LUT_DIM.w;           \
                                                                \
    Result = mix(textureLod(tex3DLUT, f3UVW, fLOD).x,textureLod(tex3DLUT, fract(f3UVW + float3(0,0,1.0/LUT_DIM.w)), fLOD).x, fQWeight);\
}