...
void main()
{
	...
	float sampleValue = length(pa-pb)/radius;

	//final color
	Color = texture(gradientTexture,vec2(sampleValue,0.5))*step(sampleValue,1.0);		//step is only required if you use mipmap or gradient texture with rgb!=0 at texCoord.t=1.0
}
